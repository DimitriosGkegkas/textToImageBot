...
export default class ViberUser extends User<IViberUser> {
    ...
    public async saveAction(action: string, text?: string): Promise<any> {
        const ActionObj = new Action({ action, id: this.id, text });
    ...
    public static async addTag(id: string, key: string): Promise<boolean> {
        try {
            await UserModel.findOneAndUpdate(
                { id },
                { $addToSet: { tag: key } }
            );
            await Tag.addTag({ key: key, name: key });
